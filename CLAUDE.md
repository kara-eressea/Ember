...
**Milestones 1–11 shipped, plus the MP mobile track, Web Push, and the MX desktop client (v0.26.0, 2026-08-06)** — the bouncer, web client, self-host hardening, profile viewer/matcher/eicon search, client polish, ads + character search, the responsive/PWA phone client, push notifications, and the Electron desktop app (embedded pglite server or thin client) are implemented and released; other post-milestone work is production bug-fix/polish rounds driven by the user's reports. Check `design/milestones.md` for current status, then work the next unchecked step (next up: the three real-device confidence passes gate v1.0 — Android push soak, iPhone re-check, `design/desktop-checklist.md`; after those, to be planned with the user — the remaining discovery-extras pool is the standing candidate). Update the tracker as steps complete. Don't trust the version in this paragraph over `git tag`/`gh release list` — check those for the truth, then update this line when cutting a release.
